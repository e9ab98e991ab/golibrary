...
func (z *ZipArchive) ExtractToFunc(destPath string, fn HookFunc, entries ...string) (err error) {
	...
	for _, f := range z.File {
		...
		if strings.HasSuffix(f.Name, "/") {
			if isHasEntry {
				if tool.IsEntry(f.Name, entries) {
...
// specified destination.
// It accepts a function as a middleware for custom operations.
func ExtractToFunc(srcPath, destPath string, fn HookFunc, entries ...string) (err error) {
	z, err := Open(srcPath)
	if err != nil {
		return err
	}
...
func (z *ZipArchive) extractFile(f *File) error {
	...
	return tool.Copy(f.tmpPath, f.absPath)
}
...
func packDir(srcPath string, recPath string, zw *zip.Writer, fn HookFunc) error {
...
func packTo(srcPath, destPath string, fn HookFunc, includeDir bool) error {
