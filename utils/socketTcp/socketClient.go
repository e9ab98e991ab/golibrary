...
const (
	CLIENT_TIME_OUT_DEFAULT = 10
	MAX_START_CLIENT        = 10
	MAX_CLIENT_POOL         = 100
	PCONNECT_KEEPALIVE_TIME = 120
)

//NewConnClient 创建普通连接
func NewConnClient(mode, host string, port int, params ...int) *ConnClientPool {
	maxStartPool := MAX_START_CLIENT
	maxClientPool := MAX_CLIENT_POOL
	l := len(params)
	if l > 0 {
		maxStartPool = params[0]
	}
	if l > 1 {
		maxClientPool = params[1]
	}
	pool := &ConnClientPool{
		mode:         mode,
		host:         host,
		...
		clientChan:   make(chan *SocketClient, maxClientPool),
	}
	pool.run(maxStartPool)
	return pool
}

//NewPconnectClient 创建长连接
func NewPconnectClient(mode, host string, port int, params ...int) *PconnectClientPool {
	maxStartPool := MAX_START_CLIENT
	maxClientPool := MAX_CLIENT_POOL
	l := len(params)
	if l > 0 {
...
func (this *ConnClientPool) GetPort() int {
	return this.port
}
...
func (this *ConnClientPool) run(maxStartPool int) {
	//this.clientChan = make(chan *SocketClient, this.poolNum)
	//预开启子协程
	for i := 0; i < maxStartPool; i++ {
...
func (this *PconnectClientPool) GetPort() int {
	return this.port
}
...
func (this *PconnectClientPool) Conn() (ClientPooler, error) {
	client := this.client()
	defer this.recyc(client)
	if _, err := client.Pconnect(this.keepAliveTime); err != nil {
		return this, err
	}
	return this, nil
}
...
func (this *PconnectClientPool) run(maxStartPool int) {
	//this.clientChan = make(chan *SocketClient, this.poolNum)
	//预开启子协程
	for i := 0; i < maxStartPool; i++ {
		this.clientChan <- &SocketClient{
...
//长连接
func (this *SocketClient) Pconnect(keepAlive ...int) (*SocketClient, error) {
	//defer this.Recover()
	if conn, e := this.connect(); e != nil {
		return this, e
	} else {
		if this.isPconnect == false {
			send := this.fiexLeninfo("pconnect", 10)
			keepAliveTime := PCONNECT_KEEPALIVE_TIME
			if len(keepAlive) > 0 && keepAlive[0] > 0 {
				keepAliveTime = keepAlive[0]
			}
			send = append(send, this.fiexLeninfo(strconv.Itoa(keepAliveTime), 5)...)
			var rsCode int
			...
			if rsCode != 200 {
				...
				fmt.Println("Pconnect successfull!")
...
//连接
func (this *SocketClient) connect() (*net.TCPConn, error) {
	var e error
	if this.conn == nil {
		rAddr := &net.TCPAddr{
			IP:   net.ParseIP(this.ClientPool.GetHost()),
			Port: this.ClientPool.GetPort(),
			Zone: this.ClientPool.GetMode(),
		}
		conn, err := net.DialTCP(this.ClientPool.GetMode(), nil, rAddr)
		if err != nil {
